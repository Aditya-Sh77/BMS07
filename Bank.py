...
con=mysql.connector.connect(host='localhost',user='root',password='admin',database='NewBank')
cursor=con.cursor()
ctr=0
bank=1
while bank==1:
    print('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>')
    print("WELCOME TO BANK OF NEXTGEN ")
    print('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>')
    print('Press 1 for Banking Services')
